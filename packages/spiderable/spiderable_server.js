...
Spiderable._getPhantomjsBinaryPath = function () {
...
WebApp.connectHandlers.use(function (req, res, next) {
  ...
  if (/\?.*_escaped_fragment_=/.test(req.url) ||
    ...
    var phantomjsPath = Spiderable._getPhantomjsBinaryPath();
    var filename = '/tmp/meteor_'+crypto.randomBytes(4).readUInt32LE(0);
    fs.writeFileSync(filename, phantomScript);
    child_process.execFile(
      '/bin/bash',
      ['-c',
       ("exec " + phantomjsPath + " --load-images=no --ignore-ssl-errors=yes " + filename)],
